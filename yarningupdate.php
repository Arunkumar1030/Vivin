...
$result = mysqli_query($conn, "SELECT * FROM yarning WHERE batchid = $batchid");
...
?>
  ...
  <div class="col-lg-7">
         <div class="card">
           <div class="card-body">
           <div class="card-title">Yarning Update Form</div>
           <hr>
           ...
           <div class="form-group">
            <label for="input-3">Scale</label>
             <select  class="form-control" id="input-3" placeholder="Enter scale" name="scale">
              <option  class="form-control" id="input-3"  hidden><?php echo $scale; ?></option>
              <option>Ton</option>
              <option>kg</option>
...
if (isset($_POST['submit'])) {
   ...
   if (empty($batchid)   || empty($qty) || empty($scale) || empty($startdate) || empty($enddate) || empty($status)) {
      if (empty($batchid)) {
    ...
    if($status == "Process Complete"){
      echo "As";
      mysqli_query($conn,"INSERT INTO weaving (batchid,process,material,qty,scale, startdate,enddate,status)
     VALUES ('$batchid','weaving', 'dress', '$qty', 'meter', '$enddate','','On Process')");
    }
      $result = mysqli_query($conn, "UPDATE yarning SET `batchid` = '$batchid', `process` = 'yarning', `material` = 'yarn', `qty` = '$qty', `scale` = '$scale', `startdate` = '$startdate', `enddate` = '$enddate', `status` = '$status' WHERE `batchid` = $batchid");
...
?>
         ...
         <button type="submit" value="Back" name="back" class="btn btn-light px-5" onclick="window.location.href = 'yarningreport.php';"><i class="icon-menu"></i>Back</button> 
         </div>
         </div>
      </div>
