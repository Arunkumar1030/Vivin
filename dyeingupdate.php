...
$sql = "SELECT * FROM `dyeing` WHERE `product id` = $productid";
...
if (isset($_POST['submit'])) {
   ...
   if (empty($batchid)   || empty($qty) || empty($qty)   || empty($startdate) || empty($enddate) || empty($status)) {
      if (empty($batchid)) {
    ...
    if($status == "Process Complete"){
      echo "As";
      mysqli_query($conn,"INSERT INTO `striching`(`batchid`, `process`, `color`, `size`, `material`, `qty`, `scale`, `startdate`, `enddate`, `status`) VALUES ('$batchid','striching', '$color', '','', '$qty', '', '$enddate','','On Process')");
    }
      $result = mysqli_query($conn, "UPDATE `dyeing` SET `batchid` = $batchid,`color` = '$color', `process` = 'dyeing', `material` = 'dress', `qty` = '$qty', `scale` = 'meter', `startdate` = '$startdate', `enddate` = '$enddate', `status` = '$status' WHERE `product id` = $productid");
